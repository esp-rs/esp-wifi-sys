...
pub const ESP_BT_CONTROLLER_CONFIG_MAGIC_VAL: u32 = 539232034;
...
pub struct esp_bt_controller_config_t {
    ...
    #[doc = "< Indicates UART number if using UART1/2 as HCI I/O interface. Configurable in menuconfig"]
    pub hci_uart_no: u8,
    #[doc = "< Indicates UART baudrate if using UART1/2 as HCI I/O interface. Configurable in menuconfig"]
    pub hci_uart_baudrate: u32,
    #[doc = "< Scan duplicate filtering mode. Configurable in menuconfig"]
    pub scan_duplicate_mode: u8,
    #[doc = "< Scan duplicate filtering type. Configurable in menuconfig"]
    ...
    #[doc = "< Maximum number of Mesh ADV packets in scan duplicate filtering list. Configurable in menuconfig"]
    ...
    #[doc = "< SCO data path, i.e. HCI or PCM module. Configurable in menuconfig"]
    pub bt_sco_datapath: u8,
    #[doc = "< True if BLE auto latency is enabled, used to enhance Classic Bluetooth performance; false otherwise. Configurable in menuconfig"]
    pub auto_latency: bool,
    #[doc = "< True if BR/EDR Legacy Authentication Vendor Specific Event is enabled, which is required to  protect from BIAS attack; false otherwise. Configurable in menuconfig"]
    ...
    pub ble_sca: u8,
    #[doc = "< PCM role (master & slave). Configurable in menuconfig"]
    pub pcm_role: u8,
    #[doc = "< PCM polar trig (falling clk edge & rising clk edge). Configurable in menuconfig"]
    pub pcm_polar: u8,
    #[doc = "< Physical shape of the PCM Frame Synchronization signal (stereo mode & mono mode). Configurable in menuconfig"]
    pub pcm_fsyncshp: u8,
    #[doc = "< True if using high level interrupt; false otherwise. Configurable in menuconfig"]
    pub hli: bool,
    #[doc = "< Scan duplicate filtering list refresh period in seconds. Configurable in menuconfig"]
    pub dup_list_refresh_period: u16,
    #[doc = "< True if BLE scan backoff is enabled; false otherwise. Configurable in menuconfig"]
...
#[doc = " @brief Vendor HCI (VHCI) callback functions to notify the Host on the next operation"]
#[repr(C)]
#[derive(Copy, Clone)]
pub struct esp_vhci_host_callback {
    #[doc = "< Callback to notify the Host that the Controller is ready to receive the packet"]
    pub notify_host_send_available: ::core::option::Option<unsafe extern "C" fn()>,
    #[doc = "< Callback to notify the Host that the Controller has a packet to send"]
